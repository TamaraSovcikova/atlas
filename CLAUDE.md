...
2026-05-30: Phase 4 UI/UX revamp shipped. Through Phases 2-3 the app gained touch games, a settings system (Playful/Balanced/Serious), and a shared content bank (88 concepts across all 11 eras, generated by Claude Code on the Max plan, fetched identically by every user, no API cost). Phase 4 adds a real design system (deeper palette, gradient accent, glow/depth, motion), a real dnd-kit drag timeline with clear feedback, and a rebuilt glowing Constellation that lives as a pull-down vaul top-banner in sessions plus a full-screen explore view from the header/home. Recall variety rotates by session position so a cold start shows a mix. Sync and Workers AI judging remain deferred.

Libraries (all MIT): @dnd-kit, motion (LazyMotion `m`), vaul, react-force-graph-2d, dexie, ts-fsrs, zustand.

KNOWN FOLLOW-UP: main entry chunk ~183KB gzip after the revamp. Acceptable for a cache-once PWA; worth code-splitting games/drag/vaul behind routes later to cut first paint.
...
- **Constellation**: `Constellation.tsx` (react-force-graph-2d, hand-drawn canvas glow keyed to `DOMAIN_HUE`) is lazy-imported by `ConstellationBanner.tsx` (in-session vaul top drawer) and `ConstellationScreen.tsx` (full screen). Never import it on a first-paint path.
