...
2026-07-19 (Chat #24): **3 features + full review gauntlet** (commit 6b41c79 build; 5b4efcb d6af3ac 23ca4c4 design; 8d3ae19 README. main pushed. Live: pages d72026a7, worker a53d3254. 115 tests + type-check clean.). (A) **Mojibake repair** `src/lib/cleanText.ts` (`fixMojibake`, reverses UTF-8-as-cp1252/Latin-1). `/review` caught the first cut CORRUPTING valid text (accent + NBSP/°/²/curly-quote is valid UTF-8) — leads now restricted to 0xC2/0xC3/0xE2. (B) **E8 behavioural placement** new `src/lib/placement.ts` — `behaviouralAnchorBonus` accelerates ONLY the 'new' ceiling from strong FSRS perf on tier-1 anchors; 'some'/'confident' bit-identical. Over-claim demotion deferred. (C) **AI recall questions** — `generateConcept`/`generateDeeperConcepts` return a validated cloze -> `Lesson` (rides community payload, worker id-opaque); lesson-less concepts get a synthesized "name it from its summary" contrast card via `makeSynthRecallItem`. `/review` found the synth card was showing its own answer (Brief renders `concept.name`; reused `isFallback`) -> **poisoned FSRS**; fixed with an `isSynth` flag + `Brief variant="stimulus"`, `policy.contrast` respected, 3+ distractors, dailyPlan resume fixed. Community `recallQuestions` now go through `sanitizeRecallQuestions` (pull path was unvalidated from an unauth endpoint). **Worker upgrade-only upsert** so an already-banked concept can gain questions (was `ON CONFLICT DO NOTHING`) — deployed, NOT verified live. (D) **Design:** DESIGN.md rewritten (feed-is-home; the constellation-hero directive was stale + a trap); onboarding `min-h-full`->`min-h-dvh` (had collapsed to 407px); Skip button 44px + AA contrast; recall stimulus unboxed. (E) **README** rewritten to actually work (`nvm use` failed without `source ~/.nvm/nvm.sh`; dead docs pointer). **Deferred (see TODOS P2):** verify the worker upsert on local D1; add fake-indexeddb (community.ts + makeRecallItem untested); perf of buildMcqDistractors on the synth feed path. **Open security (task chips, pre-existing):** unauth `POST /concepts`; `wikipediaUrl` accepts `javascript:`. gstack upgraded to 1.60.1.0 (`./setup` didn't run — bun missing).
